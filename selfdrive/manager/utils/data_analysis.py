...
'2023-09-22-01-20-39=stanley',
'2023-09-22-00-56-15-mpc',
'2023-09-22-00-43-41-mpc',
'2023-09-22-00-23-57-mpc',
...
for i in range(7):
    ...
    with rosbag.Bag(rosbag_file_name+'.bag', 'r') as bag:
        for topic, msg, t in tqdm(bag.read_messages(topics=['/veh_pose', '/mobinha/planning/local_path', '/mobinha/planning/local_path_theta', '/novatel/oem7/odom', '/mobinha/car/mode']),desc="Reading Bag"):
            ...
            if topic == '/mobinha/planning/local_path_theta':
                theta_data = msg.data  # Convert to radian if it's in degree
    ...
    df.to_json(f'../data/{rosbag_file_name}_df.json', orient="records", lines=True, date_format="iso")

    with open(f'../data/{rosbag_file_name}_accumulate_path_data.json', 'w') as file:
        json.dump(accumulated_path_data, file)
    ...
    for enu_x, enu_y, enu_heading, _ in tqdm(enu_pose_data, desc="Calculating CTE and Heading Error"):
        ...
        # print(closest_point[2], enu_heading)
        heading_errors.append(heading_error)


    with open(f'../data/{rosbag_file_name}_cte_values.json', 'w') as file:
        json.dump(cte_values, file)

    with open(f'../data/{rosbag_file_name}_heading_errors.json', 'w') as file:
        json.dump(heading_errors, file)
