...
import time
import utm
import config.zipper_config as zipper_config
...
os.environ["ZMQ"] = "1"
ZONE = zipper_config.kcity  # TOR zone
...
class LaneCheck:
    def __init__(self):
        ...
        self.lane_state_pub = rospy.Publisher(
            '/unstable_lane', Bool, queue_size=1)
        ...
        rospy.Subscriber('/can_record', Int16MultiArray,
                         self.can_record_callback)
        ...
        if(time.time() - self.sm.rcv_time['modelV2'] > 1):
            ...
            if self.sm['modelV2']:
                if self.sm['modelV2'].laneLines:
                    ...
                    # return poltnomial coefficient
                    left_fit_cr = np.polyfit(xx, lefty, 2)
                    right_fit_cr = np.polyfit(xx, righty, 2)
                    left_curvated = min(int(((1+(2*left_fit_cr[0]+left_fit_cr[1])**2)**1.5)/np.absolute(
                        2*left_fit_cr[0])), 30000)  # calculate curvature
                    right_curvated = min(int(
                        ((1+(2*right_fit_cr[0]+right_fit_cr[1])**2)**1.5)/np.absolute(2*right_fit_cr[0])), 30000)
    ...
    def get_lane_prob(self):
        if self.sm['modelV2'].laneLineProbs:
            prob = (self.sm['modelV2'].laneLineProbs[1] +
                    self.sm['modelV2'].laneLineProbs[2])/2
