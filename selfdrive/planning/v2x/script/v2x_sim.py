...
class V2X:
    ...
    def spat(self, data):
        ...
        for sections in data['intersections']:
            ...
            for states in data['intersections'][0]['states']:
                ...
                if (sections['name'] == 'World Cup Park 207 dong' or sections['name'] == 'Sangam Police Station' 
                    or sections['name'] == 'Sangam Elementary School SUB') and states['signalGroup'] == 50 and states['movementName'] != 'PED':
                    print('signal 50 : {0} , minEndTime : {1}'.format(eventstate,timing))
                    print(states['movementName'])                 
                    spat_msg.data.extend([es_int,timing])  
                elif (sections['name'] == '407 World Cup Park' or sections['name'] == 'Sangam Savoy City DMC') and states['signalGroup'] == 70 and states['movementName'] != 'PED':
                    print('signal 70 : {0} , minEndTime : {1}'.format(eventstate,timing))
                    print(states['movementName'])
                    spat_msg.data.extend([es_int,timing]) 
                elif (sections['name'] == 'Sangam MBC SUB' or sections['name'] == 'Sangam SBS') and states['signalGroup'] == 80 and states['movementName'] != 'PED':
                    print('signal 80 : {0} , minEndTime : {1}'.format(eventstate,timing))
                    print(states['movementName'])
                    spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'Digital Media City' and states['signalGroup'] == 60 and states['movementName'] != 'PED':
                    if self.heading >= 270 and self.heading <= 330:
                        print('signal 60 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'Digital Media City' and states['signalGroup'] == 70 and states['movementName'] != 'PED':
                    if self.heading >= 340 or self.heading <= 50:
                        print('signal 70 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'World Cup Park 2 Complex' or sections['name'] == 'Sangam Elementary School':
                    if states['signalGroup'] == 50 and states['movementName'] != 'PED':
                        print('signal 50 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                    if states['signalGroup'] == 60 and states['movementName'] == 'PED':
                        print('signal 60 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'World Cup Park 4 Complex' or sections['name'] == 'World Cup Park 5 Complex':
                    if states['signalGroup'] == 70 and states['movementName'] == 'PED':
                        print('signal 70 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        es_int_70 = es_int
                        timing_70 = timing
                    if states['signalGroup'] == 60 and states['movementName'] != 'PED':
                        ...
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'Sangam MBC':
                    if states['signalGroup'] == 70 and states['movementName'] != 'PED':
                        print('signal 70 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data = [0,0]
                        spat_msg.data.extend([es_int,timing])
                    if states['signalGroup'] == 80 and states['movementName'] == 'PED':
                        print('signal 80 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
                elif sections['name'] == 'Seongam three-way street':
                    if states['signalGroup'] == 80 and states['movementName'] != 'PED':
                        print('signal 80 : {0} , minEndTime : {1}'.format(eventstate,timing))
                        print(states['movementName'])
                        spat_msg.data.extend([es_int,timing])
            if sections['name'] == 'World Cup Park 4 Complex' or sections['name'] == 'World Cup Park 5 Complex':
                spat_msg.data.extend([es_int_70,timing_70])
                
        if len(spat_msg.data) == 2:
            spat_msg.data.extend([0,0])
        if len(spat_msg.data) == 4:
            spat_msg.data.extend([0,0])
    ...
    def odom_cb(self, msg):
        ...
        Y = odom[1]
        yaw = odom[2]
        self.gps.latitude, self.gps.longitude, _ = pymap3d.geodetic2enu(Y, X, 0, 126.88227370105241, 37.5833501770936, 35.61377729712613)
